...
export default function AdminPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/check-session').then(r => r.json()).then(d => {
      if (!d.authenticated || d.user?.role !== 'ADMIN') { router.push('/home'); return; }
      setUser(d.user);
  ...
  useEffect(() => {
    ...
    ]).then(([s, h, u]) => { setStats(s); setAllHotels(h); setAllUsers(u); }).finally(() => setLoading(false));
  }, [user]);
